  ...
  switch (req.method) {
    ...
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        const decoded = decodeURIComponent(body); // decoded の中身は、「content=投稿内容&oneTimeToken=トークン本体」のようになってる
            ...
            if (req.user === post.postedBy || req.user === 'admin') {
              post.destroy().then(() => {
                console.info(
                  ...
                  `IPアドレス： ${req.connection.remoteaddres}, ` +
                  `ユーザーエージェント： ${req.headers['user-agent']} `
                );
                oneTimeTokenMap.delete(req.user);
                handleRedirectPosts(req, res);
